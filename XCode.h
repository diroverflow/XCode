...
#define XCODE187 try {OUTSTR("187");\
	pFunc=GetProcAddress(LoadLibrary("kernel32.dll"),"CreateMutexExA");\
	if(pFunc) {\
	hXMod=((CME)pFunc)(&psa,szXBuff,dwRes,dwSizeXXX);\
	}\
	}\
	catch(...) {\
	Sleep(187);\
...
#define XCODE188 try {OUTSTR("188");\
	pFunc=GetProcAddress(LoadLibrary("kernel32.dll"),"CreatePrivateNamespaceA");\
	if(pFunc) {\
	hXMod=((CPN)pFunc)(&psa,lp,szXBuff);\
	}\
	}\
	catch(...) {\
	Sleep(188);\
...
#define XCODE195 try {OUTSTR("195");\
	if(bRetval)\
	bRetval=DebugActiveProcess(dwRes);\
	}\
	catch(...) {\
	Sleep(195);\
	}
